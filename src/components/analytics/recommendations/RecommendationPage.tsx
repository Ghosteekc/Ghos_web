...
export function RecommendationsPanel() {
  const [profile, setProfile] = useState<Profile | null>(() => cacheGet<Profile>("profile-v6"));
  ...
  const [loading, setLoading] = useState(
    () => !cacheHas("profile-v6") || !cacheHas("player-collection-v12"),
  );
  ...
  const load = useCallback(async () => {
    const needProfile = !cacheHas("profile-v6");
    ...
    try {
      const [profileData, collectionData] = await Promise.all([
        needProfile ? api.getProfile() : Promise.resolve(cacheGet<Profile>("profile-v6")),
        needCollection
